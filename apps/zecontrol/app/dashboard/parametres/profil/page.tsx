  ...
  return (
    <div className="dashboard-settings-page">
      ...
      <div className="settings-layout">
        <ProfileSettingsForm profile={access.profile} config={access.productProfile} />
        <aside className="settings-aside">
          <section className="settings-summary-card">
            ...
            <div><small>Rôle</small><strong>{roleLabels[access.productProfile.role]}</strong></div>
            <div><small>Politique</small><strong>{policyLabels[access.productProfile.policy]}</strong></div>
            <div><small>Travail à distance</small><strong>{access.productProfile.can_remote ? "Autorisé" : "Non autorisé"}</strong></div>
            <div><small>Organisation</small><strong>{access.organisation.name}</strong></div>
          </section>
          <div className="settings-tip"><KeyRound size={18} /><p>Votre rôle, votre politique et votre identifiant sont protégés. Un propriétaire ou un administrateur les gère depuis l’espace Équipe.</p></div>
