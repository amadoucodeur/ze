...
export function ReportPeriodToolbar({
  ...
  return (
    <div className="report-period-toolbar">
      <div className="report-period-presets" aria-label="Période du rapport">{periods.map((item) => <button className={period === item.value ? "active" : ""} type="button" onClick={() => onPeriodChange(item.value)} key={item.value}>{item.label}</button>)}</div>
