...
import { ArrowRight, Building2, Check, Circle, Sparkles, UserPlus, UserRoundSearch, Users } from "lucide-react";
    ...
    <div className="dashboard-home">
      ...
      {!profile.organisation ? <div className="dashboard-alert"><Building2 size={21} /><div><strong>Votre organisation n’est pas encore disponible</strong><p>Demandez à son propriétaire de terminer la configuration avant de commencer.</p></div></div> : <>
        <section className="activation-hero" aria-labelledby="activation-title">
          <div className="activation-copy"><span className="section-kicker">{hasFirstTalent ? "Vivier actif" : "Première étape utile"}</span><h2 id="activation-title">{hasFirstTalent ? "Continuez à construire un vivier qui travaille pour vous." : "Ajoutez votre premier talent, simplement."}</h2><p>{hasFirstTalent ? "Centralisez les profils utiles à votre organisation. La recherche intelligente s’appuiera sur ce vivier structuré." : "Commencez par les informations essentielles. Vous pourrez enrichir le profil à tout moment, sans bloquer votre travail."}</p><div className="activation-actions">{canCreateTalent && <Link className="button button-primary" href="/dashboard/talents/nouveau"><UserPlus size={18} /> {hasFirstTalent ? "Ajouter un talent" : "Ajouter mon premier talent"}</Link>}<Link className="button button-ghost" href="/dashboard/talents"><UserRoundSearch size={18} /> Voir le vivier <ArrowRight size={17} /></Link></div></div>
